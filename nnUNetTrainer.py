...
class nnUNetTrainer_JawCyst(object):
    def __init__(self, plans: dict, model: str, deepSupervised: int, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.num_epochs = 800
        self.model = model
        self.deepSupervised = deepSupervised
        self.num_class = 2
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # waveFormer \ TransBTS\
            if self.configuration_name == "2d":
                # The basic model nnUNet               
                if self.model == "PR_SDMLP":
                    from  nnunetv2.training.nnUNetTrainer.others.CNN_UNet.our_MLP.PR_SDMLP import PR_SDMLP,PR_SDMLPv2
                    self.network = PR_SDMLP(self.num_input_channels, self.num_class).to(
                        self.device)
